...
TRSensors::TRSensors() {
    ...
    SPI.begin(Clock, DataOut, Address);
...
// surface or a void).
// méthode spi avec bit banging (obsolète si SPI validé)
...
/*
 méthode à la louche avec le COVID !!!
 */

void TRSensors::AnalogReadSPI(unsigned int *sensor_values) {

    uint16_t ad;
    uint8_t ad_l;
    uint8_t chx,i;
    unsigned int values[] = {0, 0, 0, 0, 0, 0, 0, 0};
    for (chx = 0; chx < _numSensors + 2; chx++) {
        digitalWrite(CS, LOW);
    ...
    Nbat=values[i+1]; //pont diviseur d'origine mal calculé Vbat =8V soit 4V pour la mesure avec un Vref de 3.3V donc R10<6.8k
}
